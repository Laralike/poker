...
class FourOfAKindPairPlus extends Hand {
	...
	solve() {
		...
		if (this.cards.length >= 6) {
			var type = this.cards[0].toString().slice(0, -1) + "'s over " + this.cards[4].toString().slice(0, -1) +
				"'s";
...
class ThreeOfAKindTwoPair extends Hand {
	...
	solve() {
		...
		if (this.cards.length >= 7) {
			var type = this.cards[0].toString().slice(0, -1) + "'s over " + this.cards[3].toString().slice(0, -1) +
				"'s & " + this.cards[5].value + "'s";
			this.descr = this.name + ", " + type;
		}
...
class ThreePair extends Hand {
	...
	solve() {
		...
		if (this.cards.length >= 6) {
			var type = this.cards[0].toString().slice(0, -1) + "'s & " + this.cards[2].toString().slice(0, -1) +
				"'s & " + this.cards[4].toString().slice(0, -1) + "'s";
			this.descr = this.name + ", " + type;
		}
		...
		if (handValue === FiveOfAKind) {
			...
			if (altGame.handValues[altRank] === FourOfAKind) {
				...
				if (altHand.cards[0].rank < 6) {
					...
					altHand.cards[0].wildValue !== "A" && altHand.cards[2].rank < 6 &&
				...
				if (
					altHand.cards[0].rank >= values.indexOf("T") && altHand.cards[0].rank <= values.indexOf("K") &&
			...
			if (
				this.baseHand.cards[3].wildValue === "2" && this.baseHand.cards[5].wildValue === "A" &&
			...
			if (this.baseHand.cards[0].rank < 6) {
				...
				this.baseHand.cards[0].wildValue !== "A" && this.baseHand.cards[2].rank < 6 &&
		...
		for (var i = 0; i < handsToCheck.length; i++) {
			...
			if (hand.isPossible) {
				...
				if (possibleLoCards) {
					...
					if (
						!bestLoCards || bestLoCards[0].rank < possibleLoCards[0].rank ||
						(bestLoCards.length > 1 && bestLoCards[0].rank === possibleLoCards[0].rank &&
